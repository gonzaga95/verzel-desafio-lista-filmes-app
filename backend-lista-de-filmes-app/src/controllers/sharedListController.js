...
async function toggleShareStatus(req, res) {
  ...
  if (typeof isActive !== 'boolean') {
    return res
      .status(400)
      .json({ error: 'O campo isActive é obrigatório e deve ser booleano.' });
  }
  ...
  try {
    ...
    if (!sharedList) {
      return res
        .status(404)
        .json({ error: 'Lista compartilhada não encontrada.' });
    }
    ...
    res
      .status(500)
      .json({ message: 'Erro ao atualizar status de compartilhamento' });
  }
}

async function updateSharedListName(req, res) {
  const userId = req.userId;
  const { listName } = req.body;
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: 'Erro ao atualizar nome da lista compartilhada' });
  }
}
...
async function getSharedList(req, res) {
  ...
  try {
    ...
    if (!sharedList.isActive) {
      return res
        .status(403)
        .json({ error: 'Esta lista não está mais pública' });
    }
    ...
    res
      .status(500)
      .json({ message: 'Erro ao buscar informações da lista compartilhada' });
  }
}
